...
import type { SavingsCarnet, Client, UserProfile, CarnetDeposit } from '../lib/types';
import { Plus, CheckCircle, Lock, FileText, ArrowLeft, ArrowRight, Edit2, Users, FolderHeart } from 'lucide-react';
                    ...
                    {activeCarnetDetails ? (
                        <div className="panel">
                            ...
                            <div className="panel-body">
                                ...
                                <div className="slots-container" style={{ marginBottom: '32px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                                        ...
                                        <span style={{ fontSize: '13px', fontWeight: 800, color: 'var(--primary)' }}>
                                            {activeCarnetDetails.total_slots} / 31 slots remplis ({Math.round(((activeCarnetDetails.total_slots || 0) / 31) * 100)}%)
                                        </span>
                                    </div>

                                    <div className="slots-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(40px, 1fr))', gap: '8px' }}>
                                        {Array.from({ length: 31 }).map((_, index) => {
                                            const isFilled = (activeCarnetDetails.total_slots || 0) > index;
                                            return (
                                                <div
                                                    key={index}
                                                    className={`slot-item ${isFilled ? 'filled' : 'empty'}`}
                                                    style={{
                                                        height: '44px',
                                                        borderRadius: '8px',
                                                        fontSize: '12px',
                                                ...
                                                >
                                                    ...
                                                    <div className="slot-tooltip">
                                                        Case {index + 1} : {isFilled ? `${activeCarnetDetails.daily_mise.toLocaleString()} FC (Validé)` : 'Vide'}
                                                    </div>
                                                </div>
                                            );
                                        })}
                                ...
                                {/* ACTION: REQUEST WITHDRAWAL (Visible to agent when filled/locked) */}
                                {currentUser.role === 'agent' && activeCarnetDetails.status === 'locked' && (
                                    <div style={{ backgroundColor: 'rgba(99, 102, 241, 0.05)', border: '1px dashed var(--locked-border)', padding: '24px', borderRadius: '12px', textAlign: 'center' }}>
                                        <div style={{ color: 'var(--locked-color)', display: 'flex', justifyContent: 'center', marginBottom: '12px' }}>
                                            <Lock size={36} />
                                        </div>
                                        <h4 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text-dark)', marginBottom: '6px' }}>
                                            Félicitations ! Carnet rempli
                                        </h4>
                                        <p style={{ fontSize: '13px', color: 'var(--text-medium)', maxWidth: '380px', margin: '0 auto 18px' }}>
                                            Les 31 slots d'épargne ont été complétés. Le solde de {activeCarnetDetails.total_deposited?.toLocaleString()} FC est verrouillé. 
                                            Vous pouvez initier la demande de liquidation du retrait net (déduction de la mise d'adhésion).
                                        </p>
